...
//  Netflix
//
//  Created by Clara Mounir Adly on 16/01/2024.
//
    ...
    private func configureHeaderView(){
        APICaller.shared.getTrendingMovies { [weak self] result in
            ...
            case.success(let titles):
                let selectedRandom = titles.randomElement()
                self?.randomTrendingMovie = selectedRandom
                self?.headerView?.configure(with: TitleViewModel(titleName: selectedRandom?.original_title ?? "", posterURL: selectedRandom?.poster_path ?? ""))
